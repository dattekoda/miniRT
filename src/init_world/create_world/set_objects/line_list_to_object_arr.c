...
/*   Updated: 2026/01/14 00:06:53 by ikawamuk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_hitter	*line_to_object(const char *line, const t_element *object_table[]);
...
const t_element *object_table[])
{
	...
	while (line_list)
	{
		hitter_arr[i] = line_to_object(line_list->content, object_table);
		if (!hitter_arr[i])
...
t_hitter	*line_to_object(const char *line, const t_element *object_table[])
{
	...
	while (object_table[i])
	{
		if (match_identifier(line, object_table[i]) == SUCCESS)
			return (object_table[i]->line_to_hitter(line));
		i++;
	}
	return (NULL);
}
